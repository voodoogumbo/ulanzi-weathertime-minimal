...
BYU_SPORTS = {
    "basketball": f"{ESPN_BASE}/basketball/mens-college-basketball/scoreboard",
    "football": f"{ESPN_BASE}/football/college-football/scoreboard",
}
...
class TimePublisher:
    ...
    def check_byu_sports(self):
        """Check ESPN scoreboard for BYU game status across football and basketball"""
        self.game_is_live = False
        ...
        for sport_name, url in BYU_SPORTS.items():
            try:
                ...
                # Search scoreboard events for BYU's game
                byu_event = None
                for event in data.get("events", []):
                    for comp in event.get("competitions", []):
                        for c in comp.get("competitors", []):
                            if str(c.get("team", {}).get("id", "")) == BYU_TEAM_ID:
                                byu_event = event
                                break
                        if byu_event:
                            break
                ...
                competition = byu_event.get("competitions", [{}])[0]
                ...
                if state == "STATUS_IN_PROGRESS":
                    ...
                    game_date_str = byu_event.get("date", "")
                    if game_date_str:
